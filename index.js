...
app.get('/', (req, res) => {
  res.send('✅ SN Bump Bot está en línea.');
});
...
async function executeBump(interaction) {
  ...
  const bumpEmbed = new EmbedBuilder()
    ...
    .setTimestamp()
    .setFooter({ text: `SN Bump • Visibilidad: ${config.cooldownHours || 2}h` });
...
client.once('ready', async () => {
  ...
  client.user.setPresence({
    activities: [{ name: '/help | SN Bump', type: ActivityType.Watching }],
    status: 'online'
  });
  ...
  try {
    ...
    if (!clientId) {
      console.error('❌ FATAL: Falta la variable CLIENT_ID.');
    } else {
      await rest.put(Routes.applicationCommands(clientId), { body: commands });
      console.log('✅ Comandos Slash registrados exitosamente en la API de Discord.');
    }
  } catch (error) {
...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === 'help') {
    const embed = new EmbedBuilder()
      .setTitle('🤖 Panel de Ayuda - SN Bump')
  ...
  if (commandName === 'panel') {
    const embed = new EmbedBuilder().setTitle('🚀 Panel de Control - SN Bump').setDescription('Utiliza los botones a continuación para interactuar de forma rápida con el bot.').setColor('#2ECC71');
    const row = new ActionRowBuilder().addComponents(
  ...
  if (commandName === 'guia') {
    const embed = new EmbedBuilder()
      ...
      .setDescription('Pasos necesarios para poner a funcionar SN Bump en tu servidor:')
      .addFields(
...
async function checkExpiredBumps() {
  ...
  for (const guildId in db.activeBumps) {
    ...
    if (now >= bumpInfo.expiresAt) {
      ...
      delete db.activeBumps[guildId]; 
      saveDB();
    }
  }
}

// CAPTURA GLOBAL DE EXCEPCIONES Y DERECHOS DE PROCESO
...
// LOGIN CON COMPATIBILIDAD PARA DISCORD_TOKEN O TOKEN
const botToken = process.env.DISCORD_TOKEN || process.env.TOKEN;

if (!botToken) {
  console.error('❌ FATAL: No se encontró la variable de entorno DISCORD_TOKEN ni TOKEN.');
} else {
